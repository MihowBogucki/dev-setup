...
function Add-ToPersonal {
    ...
    foreach ($p in $picked) {
        ...
        $entry = [ordered]@{ name = $full.name; description = $full.description; checkCommand = $full.checkCommand }
...
function Show-GitPush {
    ...
    # Stage and commit
    git -C $RepoPath add base.json personal.json setup-cli.ps1 2>&1 | Out-Null
...
function Save-Snapshot {
    ...
    $snapshot = [ordered]@{
        _comment    = "Snapshot generated by dev-setup on $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
...
if ($menuChoice -eq -1 -or $menuChoice -eq 6) {
